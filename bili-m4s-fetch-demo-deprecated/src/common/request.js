...
export function sendFetchReq(url, options = { referer: '', origin: '', rangeStart: 0 }) {
  const { referer, origin, rangeStart } = options;
  return fetch(url, {
    headers: {
      'accept': '*/*',
...
export async function downloadM4s(url, options) {
  const { referer, origin } = options;
  const blob = await sendFetchReq(url, { referer, origin, rangeStart: 0 });
